...
namespace DotNetWeb.Core
{
    public static class Environment_Variables
    {
        ...
        public static void AddList(string lexeme, Id id) => _contexts.Last().AddList(lexeme, id);
        ...
        public class Environment
        {
            ...
            public void AddList(string lexeme, Id id)
            {
                if (!_table.TryAdd(lexeme, new Symbol(SymbolType.List, id, null)))
                {
                    throw new ApplicationException($"Variable list {lexeme} already defined in current context");
                }
            }
